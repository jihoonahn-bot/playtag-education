...
async function confirmAssign() {
  ...
  const isReassign = a.status === 'confirmed'; // 재배정 여부
  a.status        = 'confirmed';
  ...
  if (gcal.isConnected) {
    ...
    if (gId || isReassign) {
      showNotif('✅ 배정 완료 + Google 캘린더에 일정이 추가되었습니다!', 'success');
    }
  }
}
...
function rejectApp() {
  ...
  // ── Google Calendar 연동: 이미 등록된 이벤트 삭제 ──
  if (hadEvent && gcal.isConnected) {
    gcalDeleteEvent(a.id);
  }
}
...
// ════════════════════════════════════════
// GOOGLE CALENDAR — 설정 페이지 헬퍼
// ════════════════════════════════════════

/** gcal 설정 페이지 진입 시 UI 갱신 */
function initGcalSettingsPage() {
  // status dot 2개 동기화
  const dot2  = el('gcal-status-dot-2');
  const txt2  = el('gcal-status-text-2');
  if (!dot2) return;
  if (gcal.isConnected) {
    dot2.style.background = 'var(--pt-success)';
    txt2.textContent      = '연결됨';
  } else {
    const configured = GCAL_CONFIG.CLIENT_ID !== 'YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com';
    dot2.style.background = configured ? '#94A3B8' : '#FCA5A5';
    txt2.textContent      = configured ? '미연결' : 'API 키 미설정';
  }
  _updateSyncStatusUI();

  // 동기화된 일정 미리보기
  const synced = applications.filter(a => gcal.syncedIds[a.id]);
  const listWrap = el('gcal-synced-list');
  const listEl   = el('gcal-synced-items');
  if (synced.length > 0) {
    listWrap.style.display = '';
    listEl.innerHTML = synced.map(a => `
      <div style="display:flex;align-items:center;gap:10px;padding:10px;border:1px solid var(--pt-border);border-radius:9px;margin-bottom:6px;font-size:13px;">
        ${progBadge(a.program)}
        <span style="flex:1;">${a.branch} · ${formatDt(a.assignedDate)}</span>
...
function applyClientId() {
  const val = el('gcal-client-id-input')?.value.trim();
  if (!val || !val.includes('.apps.googleusercontent.com')) {
    showNotif('올바른 클라이언트 ID 형식이 아닙니다.\n(xxxxx.apps.googleusercontent.com)', 'danger');
    return;
  }
  // localStorage에 임시 저장 후 gcal.js에서 읽도록
  localStorage.setItem('pt_gcal_client_id_override', val);
  showNotif('Client ID가 임시 적용됩니다. gcal.js 파일도 동일하게 수정해주세요.', 'success');
  setTimeout(() => location.reload(), 1500);
}

/** 설정 가이드 토글 */
function toggleSetupGuide() {
  const guide = el('gcal-setup-guide');
  if (!guide) return;
