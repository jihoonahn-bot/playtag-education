...
window.gisLoaded = function () {
  const clientId = localStorage.getItem('pt_gcal_client_id_override') || GCAL_CONFIG.CLIENT_ID;
  if (clientId === 'YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com') {
    // Client ID 미설정 시 → 임시 로그인 폼 표시
    gcal.gisReady = false;
    const fb = el('login-admin-fallback');
    const hint = el('google-login-hint');
  ...
  // ── 로그인용 ID 토큰 클라이언트 ──
  gcal.idClient = google.accounts.oauth2.initCodeClient
    ? null
    : null;
  // Google One Tap / ID 토큰 초기화
  google.accounts.id.initialize({
    client_id: clientId,
    callback:  _handleIdTokenResponse,
    auto_select: false,
  });
  gcal.gisReady = true;
  _checkBothReady();
...
// ════════════════════════════════════════
// ★ Google 계정 로그인 (관리자/담당자)
// ════════════════════════════════════════

/** 로그인 화면 "Google 계정으로 로그인" 버튼 */
function loginWithGoogle() {
  const clientId = localStorage.getItem('pt_gcal_client_id_override') || GCAL_CONFIG.CLIENT_ID;
  if (clientId === 'YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com') {
    showNotif('⚙️ 먼저 API 설정에서 Google Client ID를 입력해주세요.', 'danger');
    const fb = el('login-admin-fallback');
    if (fb) fb.style.display = '';
    return;
  }
  ...
  // 로딩 표시
  const btn = el('btn-google-login');
  const loading = el('google-login-loading');
  if (btn) btn.style.display = 'none';
  if (loading) loading.style.display = '';

  // Google One Tap으로 ID 토큰 요청
  google.accounts.id.prompt((notification) => {
    if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
      // One Tap이 안 되면 팝업으로 fallback
      _requestGoogleLoginPopup();
    }
  });
}

/** 팝업 방식으로 Google 로그인 */
function _requestGoogleLoginPopup() {
  // access token + openid scope로 사용자 정보 획득
  const clientId = localStorage.getItem('pt_gcal_client_id_override') || GCAL_CONFIG.CLIENT_ID;
  const loginTokenClient = google.accounts.oauth2.initTokenClient({
    client_id: clientId,
    scope: 'openid email profile ' + GCAL_CONFIG.SCOPES,
    callback: _handleLoginTokenResponse,
  });
  loginTokenClient.requestAccessToken({ prompt: 'select_account' });
}

/** One Tap ID 토큰 응답 처리 */
async function _handleIdTokenResponse(response) {
  ...
  try {
    ...
    await _processGoogleLogin(payload.email, payload.name, payload.picture);
  } catch(e) {
    _resetGoogleLoginBtn();
    showNotif('로그인 처리 중 오류가 발생했습니다.', 'danger');
  }
}

/** 팝업 방식 토큰 응답 처리 */
async function _handleLoginTokenResponse(resp) {
  if (resp.error) {
    _resetGoogleLoginBtn();
    showNotif('Google 로그인이 취소되었습니다.', 'danger');
    return;
  }
  // 사용자 정보 조회
  try {
    ...
    // Calendar 토큰도 저장 (이미 scope에 포함됨)
    gcal.accessToken = gapi.client.getToken().access_token;
    gcal.isConnected = true;
    gcal.userEmail   = info.email;
    await _processGoogleLogin(info.email, info.name, info.picture);
  } catch(e) {
    _resetGoogleLoginBtn();
    showNotif('사용자 정보 조회에 실패했습니다.', 'danger');
  }
}
...
/** 로그인 실행 */
function _doGoogleLogin(userInfo) {
  // Google Calendar 연동도 자동 처리
  if (gcal.gisReady && gcal.tokenClient && !gcal.isConnected) {
    // 이미 access token이 있으면(팝업 방식) 그대로 사용, 없으면 연결
    if (gcal.accessToken) {
      gcal.isConnected = true;
      gcal.userEmail   = userInfo.email;
      localStorage.setItem('pt_gcal_connected_at', new Date().toLocaleString('ko-KR'));
      _updateGcalUI();
      gcalSyncAll();
    } else {
      // Calendar scope 별도 요청
      setTimeout(() => gcalConnect(), 800);
    }
  }
  ...
  _renderGoogleUserBadge(userInfo);
  setupNav();
  showNotif(`${userInfo.name}님, Google 계정으로 로그인되었습니다 ✅`, 'success');
}

/** 상단바 Google 프로필 사진 표시 */
function _renderGoogleUserBadge(userInfo) {
  const badge = el('user-badge');
  if (!badge || !userInfo.picture) return;
  const existingAvatar = badge.querySelector('.topbar-google-avatar');
  if (!existingAvatar) {
    const img = document.createElement('img');
    img.src = userInfo.picture;
    img.className = 'topbar-google-avatar';
